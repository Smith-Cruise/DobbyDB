...
| `s3-storage` | Inline table | No | Environment credentials | Credentials and endpoint used to read table data at `s3://` or `s3a://` locations. |
| `oss-storage` | Inline table | No | Environment credentials | Credentials and endpoint used to read table data at `oss://` locations. |
